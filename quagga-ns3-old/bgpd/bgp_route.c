...
bgp_info_cmp (struct bgp *bgp, struct bgp_info *new, struct bgp_info *exist)
{
  ...
  /* New. Trust check. */
  myAS=bgp->as;
  peerExistAS=exist->peer->as;
  peerNewAS=new->peer->as;
  ...
  fclose(fptr);
  newTrust = (float) (inTrusts[peerNewAS-1]+obsTrusts[peerNewAS-1]);
  existTrust = (float) (inTrusts[peerExistAS-1]+obsTrusts[peerExistAS-1]);
  u_int16_t oldvotes[10];
  u_int16_t newvotes[10];
  
  char oldstr[50];
  sprintf(oldstr, "%s-%d","voteresult", exist->peer->as);
  char newstr[50];
  sprintf(newstr, "%s-%d","voteresult", new->peer->as);
  if ((access(oldstr, F_OK ) != -1) && (access(newstr, F_OK ) != -1)) {
    FILE *oldVoting =fopen(oldstr,"r");
    fscanf(oldVoting,"%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n",
    &oldvotes[0], &oldvotes[1], &oldvotes[2], &oldvotes[3], &oldvotes[4], &oldvotes[5], &oldvotes[6], &oldvotes[7], &oldvotes[8], &oldvotes[9]);
    fclose(oldVoting);
    FILE *newVoting =fopen(newstr,"r");
    fscanf(newVoting,"%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n",
    &newvotes[0], &newvotes[1], &newvotes[2], &newvotes[3], &newvotes[4], &newvotes[5], &newvotes[6], &newvotes[7], &newvotes[8], &newvotes[9]);
    fclose(newVoting);
    float newVotedTrust = 0;
    float existVotedTrust = 0;
    float sumOldTrusts = 0;
    float sumNewTrusts = 0;
    int i;
    for (i = 0; i<10; i++) {
      if (oldvotes[i] != 0 && i != myAS -1) {
        sumOldTrusts = sumOldTrusts + inTrusts[i];
      }
    }
    for (i = 0; i<10; i++) {
      if (oldvotes[i] != 0 && i != myAS -1) {
        existVotedTrust = existVotedTrust  + (oldvotes[i] * ((float) inTrusts[i] / sumOldTrusts));
      }
    }
    for (i = 0; i<10; i++) {
        if (newvotes[i] != 0 && i != myAS -1) {
          sumNewTrusts = sumNewTrusts + inTrusts[i];
        }
      }
    for (i = 0; i<10; i++) {
      if (newvotes[i] != 0 && i != myAS -1) {
        newVotedTrust = newVotedTrust  + (newvotes[i] * ((float) inTrusts[i] / sumNewTrusts));
      }
    }
    if(newVotedTrust + newTrust > existVotedTrust + existTrust){
      zlog_warn ("Taking the new route VOTING - New: %f + %f, Old: %f + %f", newVotedTrust, newTrust, existVotedTrust, existTrust);
      return 1;
    }
    if(newVotedTrust + newTrust <= existVotedTrust + existTrust){
      zlog_warn ("Keeping the old route VOTING - New: %f + %f, Old: %f + %f", newVotedTrust, newTrust, existVotedTrust, existTrust);
      return 0;
    }
  }
  else
  {
    if(newTrust > existTrust){
      zlog_warn ("Taking the new route - New: %f, Old: %f", newTrust, existTrust);
      return 1;
    }
    if(newTrust < existTrust) {
      zlog_warn ("Keeping the old route - New: %f, Old: %f", newTrust, existTrust);
      return 0;
    }
  }
...
bgp_process_main (struct work_queue *wq, void *data)
{
  ...
  uint32_t myAS=bgp->as;
  FILE *fptr;
  if( access( "/myAS.txt", F_OK ) == -1 ) {
    fptr = fopen("/myAS.txt","w");
    fprintf(fptr,"%d",myAS);
    fclose(fptr);
  }
  if( access( "/inTrust.txt", F_OK ) == -1 ) {
    fptr = fopen("/inTrust.txt","w");
    switch (myAS){
      case 1:
        fprintf(fptr,"71\n40\n50\n60\n34\n56\n78\n31\n80\n45\n");
        break;
